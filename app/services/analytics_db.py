...
# 默认路径（环境变量 ANALYTICS_DB_PATH 可覆盖，惰性求值支持运行时切换/测试隔离）
_DEFAULT_DB_PATH = os.path.join(DATA_DIR, 'analytics.db')

_write_lock = Lock()


def _db_path():
    """惰性求值数据库路径（每次连接时读取，便于测试注入）。"""
    return os.environ.get('ANALYTICS_DB_PATH') or _DEFAULT_DB_PATH
...
try:
    init_db()
    print(f"[分析] SQLite 数据库已初始化: {_db_path()}")
