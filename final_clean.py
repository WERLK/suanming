...
.uc-back:active { background: rgba(255, 255, 255, 0.1); }
...
.vip-back:active { background: rgba(255, 255, 255, 0.1); }
...
function doRegister() {
    ...
    if (!phone || !code || !password || !password) {
        showToast('请填写完整信息');
        return;
    }
...
print("✅ 最终版 index.html 已生成")
print(f"  - 文件大小: {len(html_template)} 字符")
print("  - 所有CSS属性名已验证正确")
print("  - 所有JavaScript函数已验证正确")
print("  - 所有HTML元素ID已验证正确")
print("  - classList 和 textContent 大小写已修正")
