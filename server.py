...
def register():
    ...
    new_user = {
        ...
        "is_vip": True,  # 注册送免费会员
        "vip_expire": time.time() + 30 * 24 * 3600,  # 30天有效期
        "vip_type": "免费会员",
    ...
    return jsonify({
        ...
        "data": {
            ...
            "vip_expire": new_user['vip_expire'],
            "vip_type": new_user['vip_type']
        }
    }), 200
...
def watch_ad():
    ...
    # 计算新的过期时间
    if user.get('is_vip') and user.get('vip_expire', 0) > current_time:
        new_expire = user['vip_expire'] + one_day
    else:
        new_expire = current_time + one_day
    
    user['is_vip'] = True
    user['vip_expire'] = new_expire
    user['vip_type'] = '免费会员（广告）'
    
    # 记录广告观看历史
    if 'ad_history' not in user:
        user['ad_history'] = []
    
    user['ad_history'].append({
        "reward": "1天免费会员",
    ...
    print(f"\n【观看广告】手机号 {phone} 领取1天免费会员")
    print(f"【观看广告】有效期至：{time.strftime('%Y-%m-%d', time.localtime(new_expire))}\n")
...
# ===== 11. 获取最新验证码（测试用）=====
@app.route('/api/getLatestCode', methods=['GET'])
def get_latest_code():
    """
    获取最新发送的验证码（仅用于测试环境）
    实际生产环境不应该提供此接口
    ...
    try:
        if not os.path.exists(CODE_FILE):
            return jsonify({"code": 404, "msg": "验证码不存在"}), 200
            
        with open(CODE_FILE, 'r', encoding='utf-8') as f:
            content = f.read()
            
        import re
        phone_match = re.search(r'手机号：(\d+)', content)
        code_match = re.search(r'验证码：(\d+)', content)
        
        if not phone_match or not code_match:
            return jsonify({"code": 404, "msg": "验证码格式错误"}), 200
            
        if phone_match.group(1) != phone:
            return jsonify({"code": 403, "msg": "手机号不匹配"}), 200
        ...
        return jsonify({"code": 500, "msg": f"获取失败: {str(e)}"}), 200
